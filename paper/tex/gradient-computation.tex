...
.\end{equation*}
If we assume\footnote{It is if $ \ell $ is quadratic.}\info{Assumption} that $ h \mapsto D \risk [ h ] $ is a continuous mapping from $ L^{ 2 } ( X ) $ to $ L^{ 2 } ( X ) $\improvement{Talk about which conditions $ \ell $ can satisfy so that this is continuous.}, then $ \risk $ is also Fréchet-differentiable, and both derivatives coincide.
