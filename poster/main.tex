...
{
    \vspace{1.3em}
    {\huge \sffamily Nonparametric Instrumental Variable Regression \\ through Stochastic Approximate  Gradients}
    \vspace{.1em}
}
}
...
{
        \small{{\bfseries Caio Peixoto}$^1$, Yuri F. Saporito$^1$, Yuri R. Fonseca$^2$}
    {
        \vspace{0.1em} \\
        \small{$1.$ School of Applied Mathematics -- FGV}
        \vspace{.1em} \\
        \small{$2.$ Department of Decision, Risk and Operations -- Columbia University}
        \vspace{.5em}
    }
}
{
...
\headerbox{1. Abstract}{name=abstract,column=0,row=0, span=1}{
    We propose SAGD-IV, an algorithm which performs \highlight{\textbf{instrumental variable regression}} through iterative \highlight{SGD updates in a\\ functional space}:
    \begin{itemize}[noitemsep]
        \item IV regression is formulated as a statistical inverse problem;
        \item A suitable risk functional is introduced;
        \item This risk is minimized directly using SGD.
    \end{itemize}
...
\headerbox{5. Numerical Experiments}{name=experiment,column=0,span=2,below=npiv}{

    {
        % We compared our method with the most relevant ones in the literature:
        % \begin{center}
        %     \includegraphics[width=.72\textwidth]{graph_plots.pdf}
        %     \includegraphics[width=.26\textwidth]{mse.pdf}
        % \end{center}
        \centering
        \begin{minipage}{.65\textwidth}
        \centering
        \includegraphics[width=\linewidth]{graph_plots.pdf}
        \end{minipage}%
...
\headerbox{6. Binary Outcomes}{name=next-steps,column=0,below=experiment, above=bottom,span=1}
{
    \begin{itemize}[noitemsep]
        \item In binary outcomes settings, we have $ Y = \ind \left\{ \hstar ( X ) + \varepsilon > 0 \right\} $.
        \item We then take the loss function to be
        \vspace{-5pt}
        $$ \ell ( r(Z), \meanop [ h ] ( Z ) ) = \BCE ( r(Z), \sigma ( \meanop [ h ] ( Z ) ) ), $$
        where $ \BCE ( p, q ) = - [ p \log q + ( 1 - p ) \log ( 1 - q ) ] $ and \\ $ \sigma ( x ) = e^{ x } / ( 1 + e^{ x } ) $.

    \end{itemize}
}

\headerbox{7. References}{name=references,column=1,below=experiment,above=bottom,span=1}{
    \nocite{sagd,yuri2022,wooldridge2001,newey2003,darolles2011}
