...
Along the exposition, we will cover the basics of Two Stages Least Squares, the IV regression method most widely employed in econometric practice.%  and of the nonparametric approach developed by Newey and Powell in their seminal paper \cite{newey2003}.
...
Their new dimensions are $ d_{ X } + 1 $ and $ d_{ Z } + 1 $ but, by an abuse of notation, we will continue writing $ d_{ X } $ and $ d_{ Z } $ until the end of this section.
...
Assume that $ X : ( \Omega, \mathcal{A} ) \to ( \R^{ d_{ X } }, \mathcal{B} ( \R^{ d_{ X } } ) ) $ and $ \varepsilon : ( \Omega, \mathcal{A} ) \to ( \R, \mathcal{B} ( \R ) ) $ are measurable\footnotemark~and, furthermore, that $ \varepsilon \in L^{ 2 } ( \Omega, \mathcal{A}, \prob ) $ with $ \mean [ \varepsilon ] = 0 $.
...
Denote by $ \prob_{ X } $ the distribution of the random variable\footnote{We use the term ``random variable'' when referring to scalar or vector valued measurable functions defined on $ ( \Omega, \mathcal{A} ) $.} $ X $, that is, the pushforward measure $ X_{ * } \prob = \prob \circ X^{ -1 } $ defined on $ \mathcal{B} ( \R^{ d_{ X } } ) $.
...
\end{thm}
\begin{proof}
    See Appendix \ref{sec: proof identification}.
\end{proof}
