...
\begin{equation*}
    \meanop [ \hstar ] = r
,\end{equation*}
we introduce a pointwise loss function $ \ell : \R \times \R \to \R $ and define the associated \emph{populational risk measure} $ \risk : L^{ 2 } ( X ) \to \R $ as
\begin{equation*}
    \risk ( h ) = \mean [ \ell ( r ( Z ), \meanop [ h ] ( Z ) ) ]
.\end{equation*}
Throughout the rest of this chapter, the example the reader should keep in mind is the squared\footnotemark~loss function $ \ell ( y, y' ) = \frac{ 1 }{ 2 } ( y - y' )^2 $.
...
Our goal is to solve the NPIV regression problem stated in Section \ref{sec: problem specification} by solving
\begin{equation*}
...
We denote by $ \partial_{ 2 } $ a partial derivative with respect to its second argument.
...
.\end{equation*}
Therefore, taking $ g ( z ) = \partial_{ 2 } \ell ( r ( z ), \meanop [ h ] ( z ) ) $, we have that the random variable given by $ \Phi ( x, Z ) \partial_{ 2 } \ell ( r ( Z ), \meanop [ h ] ( Z ) ) $ is an unbiased stochastic estimate of $ \nabla \risk ( h ) ( x ) $.
...
\begin{thm}
    ...
    Then, if we let $ \hat{ h } = \frac{ 1 }{ M } \sum_{ m=1 }^{ M } \hat{ h }_{ m-1 } $, the following bound holds:
    \begin{align}
            \mean_{ \bz_{ 1:M } } \left[
    ...
    \begin{align*}
        ...
        &= 2 D \max \left\{
            ...
            2L^2 D^2 \norm{ \hat{ \Phi } }_{ \infty }^2
        \right\}, \\
        \zeta = \zeta ( \hat{ \Phi }, \hat{ r }, \hat{ \meanop } )
...
The last term appears due to the fact that we do not know $ \Phi, r $ nor $ \meanop $, but it explicitly quantifies how the estimation errors of $ \hat{ \Phi }, \hat{ r } $ and $ \hat{ \meanop } $ come together to determine the quality of the final estimator and converges to zero if the chosen estimation methods are consistent.
...
All of these estimators are based on kernel Ridge regression, where the unknown function is approximated using members of a Reproducing Kernel Hilbert Space (RKHS) --- see \cite[Chapter~4]{svm2008} for an introduction. 
...
Using a Gaussian kernel for the estimator, one can show that it adheres to Assumption \ref{assumption estimators}.
...
To estimate $ r ( Z ) = \mean [ Y \mid Z ] $, we simply used this same method to obtain an approximation to the operator $ f \in L^2 ( Y ) \mapsto \mean [ f ( Y ) \mid Z = \cdot \ ] \in L^2 ( Z ) $, which we then apply to the identity function.
...
Of course, this is not a thorough investigation of the empirical performance of SAGD-IV, nor irrefutable proof that it is better then KIV, and a more detailed analysis, involving other recent methods which have appeared in the NPIV literature, is left for future work.
